...
with open('/home/cecilia/Documentos/PIBIC/Fase2/DADOS-LLMS/test (1).json', 'r', encoding='utf-8') as f:
...
for item in dados_originais:
    ...
    termos_aspectos =item['aspect_terms']

    
    for i,aspecto in enumerate(termos_aspectos):
        tokens,tags = processamento(item['sentence'], aspecto)

        novo_exemplo = {
            "tokens": tokens,
            "tags": tags,
            'categoria': item['categories'][i]
            }

        lista_dados.append(novo_exemplo)


with open('test_bio.json', 'w', encoding='utf-8') as f:
